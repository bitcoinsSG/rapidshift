...
var exchangeStats = module.exports ={
	...
	exchangeRate : null,
	exchangeTxId : null,
	AfterExchangeStats : { incomingCoin : null , outgoingCoin : null, costForExchange : null},
...
function populateBeforeExchange(program){
	...
	exchangeStats.refundAddress = program.refund;
	exchangeStats.destinationAddress = program.destination;
	Q.all([
	...
	.then(function(){
		...
		exchangeStats.percentages.percentProposedDuringExchange = parseFloat(100*(parseFloat(exchangeStats.destinationCoin.priceProposedDuringExchange_USD)/parseFloat(exchangeStats.destinationCoin.priceFromExternal_USD)));
...
function populateAfterExchange(data){
	...
	exchangeStats.percentages.percentCostAfterExchange = parseFloat(100*(parseFloat(exchangeStats.destinationCoin.priceCostAfterExchange_USD)/parseFloat(exchangeStats.destinationCoin.priceFromExternal_USD)));
	exchangeStats.exchangeTxId = data.transaction;
	exchangeStats.AfterExchangeStats.incomingCoin = parseFloat(data.incomingCoin);
	exchangeStats.AfterExchangeStats.outgoingCoin = parseFloat(data.outgoingCoin);
    exchangeStats.AfterExchangeStats.costForExchange = parseFloat((exchangeStats.AfterExchangeStats.incomingCoin * exchangeStats.sourceCoin.priceFromExternal_USD) - (exchangeStats.AfterExchangeStats.outgoingCoin * exchangeStats.destinationCoin.priceFromExternal_USD));

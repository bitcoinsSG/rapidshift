...
function mockshapeshiftstatus(depositAddress, callback){
	...
	else if( count > (threshold-1) && count < (threshold*6)){
		count++;
		var receiveddata =  { status: 'received',  address: '1H4vW9wcgr5i6LVsYMTuz449ci7ZWSyX6F', incomingCoin: 50 ,incomingType: 'ETH' };
		callback(null,'received',receiveddata);
	}
	else if( count > ((threshold*6)-1)){
		count = 0;
